...
const pd = require("../Schema/Product");
...
router.post("/productdet", prodctimg.single("image"), async(req,res)=>{
    try {
        ...
        const{prodid,Prodtname,Prodtdescription,Prodtprice,Prodtcategory,ProdtstockQuantity,ProductCoin,qr} = req.body;
...
// GET PRODUCT USING PRODID

router.get("/productdet/:prodid", async (req, res) => {
    try {

        const prodid = Number(req.params.prodid);

        const product = await pd.findOne({ prodid });
        ...
        const products =
            await pd.find({
                ...
                $or: [
                    ...
                    {
                        prodid: isNaN(searchText)
                            ? -1
                            : Number(searchText)
                    },

                    {
                        Prodtprice: isNaN(searchText)
                            ? -1
                            : Number(searchText)
                    }
                ]
